...
public interface BiteRepository extends JpaRepository<Bite, Long> {
    ...
    @Query("SELECT b FROM Bite b WHERE b.status = :status " +
           ...
           "AND b.id NOT IN (SELECT uv.bite_id FROM user_viewed_bites uv WHERE uv.user_id = :userId " +
           "  AND uv.bite_id NOT IN (SELECT bm.bite.id FROM Bookmark bm WHERE bm.user.id = :userId)) " +
    ...
    // ── Native Queries for user_viewed_bites table ──────────────────────────
    ...
    @Query(value = "SELECT bite_id FROM user_viewed_bites WHERE user_id = :userId", nativeQuery = true)
